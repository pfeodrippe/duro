...
public:
  ...
  operator int32_t() const {
    if (word[2] & 0x80) { // negative? - then sign extend.
      return (int32_t)(((uint32_t)0xff << 24) | ((uint32_t)word[2] << 16) |
                       ((uint32_t)word[1] << 8) | ((uint32_t)word[0] << 0));
    } else {
      return (int32_t)(((uint32_t)word[2] << 16) | ((uint32_t)word[1] << 8) |
                       ((uint32_t)word[0] << 0));
    }
  }
...
template <size_t S, typename word_t = FERRET_BITSET_WORD_TYPE> class bitset {
  static const size_t bits_per_word = sizeof(word_t) * 8;
  static const size_t n_words = S / bits_per_word;

  static_assert((S % bits_per_word) == 0,
                "bitset size must be a multiple of word_t");
...
template <> struct fixed_real_container<8> {
  typedef int8_t base_type;
  typedef int16_t next_type;
};
template <> struct fixed_real_container<16> {
  typedef int16_t base_type;
  typedef int24_t next_type;
};
template <> struct fixed_real_container<24> {
  typedef int24_t base_type;
  typedef int32_t next_type;
};
template <> struct fixed_real_container<32> {
  typedef int32_t base_type;
  typedef int64_t next_type;
};
template <> struct fixed_real_container<64> {
  typedef int64_t base_type;
  typedef int64_t next_type;
};

#pragma pack(push, 1)
template <int bits, int exp> class fixed_real {
  ...
  template <typename T> inline base from(T d) const {
...
public:
  ...
  template <typename T> explicit fixed_real(T v) : m(from<T>(v)) {}

  template <typename T> fixed &operator=(T v) {
    m = from<T>(v);
    return *this;
  }

  // to types
  template <typename T> operator T() const { return to_whole<T>(); }
  ...
  fixed &operator*=(const fixed &x) {
    m = (base)(((next)m * (next)x.m) >> N);
    return *this;
  }
  fixed &operator/=(const fixed &x) {
    m = (base)(((next)m * factor) / x.m);
    return *this;
  }
...
template <typename T, typename... Ts> static constexpr T max(T a, Ts... bs) {
  return max(a, max(bs...));
}
...
template <typename T, typename... Ts> static constexpr T min(T a, Ts... bs) {
  return min(a, min(bs...));
}

template <typename T> constexpr T abs(T a) { return ((a) < (T)0 ? -(a) : (a)); }
} // namespace runtime
} // namespace ferret
...
template <> void print(const real_t &n) {
  std::cout << std::fixed << std::setprecision(real_precision) << n;
}

void read_line(char *buff, std::streamsize len) { std::cin.getline(buff, len); }
} // namespace runtime
#endif
...
template <typename T> void print(const T t) {
  FERRET_HARDWARE_ARDUINO_UART_PORT.print(t);
}
...
template <class T> size_t align_of(const void *ptr) {
...
template <class T> size_t align_req(const void *ptr) {
...
memory_pool<FERRET_MEMORY_POOL_PAGE_TYPE, FERRET_MEMORY_POOL_SIZE>
    program_memory;
...
public:
  ...
  template <typename FT> static inline void *allocate() {
    return allocate(sizeof(FT));
  }
...
template <typename rc> class object_i : public rc {
public:
  object_i() {}
  virtual ~object_i(){};
...
template <> inline seekable_i *var::cast<seekable_i>() const {
  return obj->cast_seekable_i();
}

template <typename rc> bool object_i<rc>::equals(ref o) const {
  return (this == o.get());
}
...
#endif
template <typename FT, typename... Args> inline var obj(Args... args) {
  void *storage = FERRET_ALLOCATOR::allocate<FT>();
  return var(new (storage) FT(args...));
}

inline var nil() { return var(); }
#undef alloca
...
public:
  template <typename... Args> inline explicit alloca(Args... args) {
...
public:
  inline matrix(size_t r = 0, size_t c = 0) : data(r * c), rows(r), cols(c) {}
  ...
  inline matrix(matrix &m) : matrix(m.rows, m.cols) {
...
namespace runtime {
var list(ref v);
template <typename... Args> var list(ref first, Args const &... args);
...
#define for_each(x, xs)                                                        \
  for (var _tail_ = rt::rest(xs), x = rt::first(xs); !_tail_.is_nil();         \
...
#if !defined(FERRET_DISABLE_STD_OUT)
  void stream_console() const final { rt::print(n); }
#endif

  template <typename T> explicit number(T x) : n(real_t(x)) {}

  template <typename T> static T to(ref v) { return (T)v.cast<number>()->n; }
};
#endif
...
#if !defined(FERRET_DISABLE_STD_OUT)
  void stream_console() const final { rt::print("()"); }
#endif
};
...
namespace runtime {
inline var list() { return cached::empty_sequence_o; }
inline var list(ref v) { return obj<sequence>(v, cached::empty_sequence_o); }

template <typename... Args> inline var list(ref first, Args const &... args) {
...
public:
  ...
  explicit lazy_sequence(ref d, ref t, bool c = false)
      : cache(c), thunk(t), data(d) {}
...
template <> class array<var> {
  ...
  var *allocate() {
    var *storage =
        static_cast<var *>(FERRET_ALLOCATOR::allocate(_size * sizeof(var)));
    for (size_t i = 0; i < _size; i++)
      new (&storage[i]) var();
    return storage;
  }
...
template <> class array_seq<var, var> : public object, public seekable_i {
  size_t pos{0};
...
public:
  ...
  template <typename... Args> static inline var into(Args... rest) {
...
namespace runtime {
template <typename... Args> static inline var dense_list(Args... rest) {
...
#if !defined(FERRET_DISABLE_STD_OUT)
  void stream_console() const final { data.stream_console(); }
#endif
...
template <> inline var obj<d_list>(var keys, var vals) {
...
public:
  ...
  explicit string(const char *str, number_t length) {
    from_char_pointer(str, length);
  }
...
#ifdef FERRET_STD_LIB
template <> inline var obj<string>(std::string s) {
...
#ifdef FERRET_HARDWARE_ARDUINO
template <> inline var obj<string>(String s) {
  void *storage = FERRET_ALLOCATOR::allocate<string>();
...
#if defined(FERRET_HARDWARE_ARDUINO)
  inline unsigned long now() const { return ::micros(); }
...
#if defined(FERRET_HARDWARE_ARDUINO)
  inline void _wait(real_t t) const { ::delayMicroseconds((number_t)t); }
#elif defined(FERRET_STD_LIB)
...
public:
  explicit fn_throttler(var f, real_t r, bool b)
      : fn(f), rate(r), blocking(b) {}
...
template <typename T> class pid_controller : public lambda_i {
  mutex lock;
  mutable T setpoint;
...
public:
  pid_controller(T kp, T ki, T kd, T inMin, T inMax, T outMin, T outMax,
                 bool cont, ref sp)
      : p(kp), i(ki), d(kd), maximum_output(outMax), minimum_output(outMin),
        maximum_input(inMax), minimum_input(inMin), continuous(cont) {
  ...
  var invoke(ref args) const final {
    if (!setpoint_fn.is_nil())
      var((object *)this)
          .cast<pid_controller<T>>()
          ->set_setpoint(run(setpoint_fn));

    return var((object *)this).cast<pid_controller<T>>()->step(rt::first(args));
  }
};
#endif
...
template <typename T> class moving_average_filter : public lambda_i {
  mutex lock;
  T alpha;
  mutable T avrg;
...
public:
  ...
  var invoke(ref args) const final {
    return var((object *)this)
        .cast<moving_average_filter<T>>()
...
template <> inline var run(ref fn) {
  return fn.cast<lambda_i>()->invoke(nil());
}

template <typename... Args> inline var run(ref fn, Args const &... args) {
...
inline var print::invoke(ref _args_) const {
  (void)(_args_);
  ref more = _args_;
...
#error print Not Supported on This Platform
#endif
  ;
}

inline var FN__3367::invoke(ref _args_) const {
  (void)(_args_);
...
inline var println::invoke(ref _args_) const {
  (void)(_args_);
  ref more__3098 = _args_;

  run(apply(), obj<print>(), more__3098);
  return run(FN__3367());
}

inline var FN__3369::invoke(ref _args_) const {
  (void)(_args_);
...
inline var FN__3370::invoke(ref _args_) const {
  (void)(_args_);
...
inline var FN__3371::invoke(ref _args_) const {
  (void)(_args_);
